...
#define backwards 0
#define pi 3.1416
#define Kp_move 0.4   // need tuning
#define Ki_move 0
#define Kd_move 0
#define Max_delta_angle 3.1416 //180 degree
#define Kp_curve 0.6  // red same side 0.6
#define Ki_curve 0
#define Kd_curve 0// red same side 0
...
void move(int x, int y){
	...
	//if(drive_PID < 0) {drive_PID = 0;}
	//if(drive_PID > 1) {drive_PID = 1;}
    //if(fabs(delta_angle) > Max_delta_angle) {
	//    if(delta_angle < 0) 
	//	     {left_on(drive_PID, forwards);
	//	      right_on(drive_PID, backwards);} 
	//	else {left_on(drive_PID, backwards);
